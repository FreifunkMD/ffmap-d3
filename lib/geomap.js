...
function update(data) {
  ...
  var routers = new_nodes.append("circle")
           .attr("r", function(d){
						 return 3 + Math.log(1 + d.clientcount / 2) + "pt"
					 })
           .attr("fill", function (d) {
             if(!d.flags.online)
               return "rgba(128, 128, 128, 0.2)";
             return (d.firmware >= ffmapConfig.current_firmware) ?"rgba(0, 255, 0, 0.8)":"rgba(10, 150, 255, 1.0)";
           })
           .on("click", function(n) { window.location.href = "graph.html#" + n.id })
           .append("title").text( function (n) { return (n.name?n.name:n.id) + ( n.clientcount ? (" (" + n.clientcount + ")") : "" )})
  ...
  */
  ...
  label_g.append("g").each( function (d) {
    ...
    d.nodes.forEach( function (n, i) {
      ...
      o.append("text").text(name + ( n.clientcount ? (" (" + n.clientcount + ")") : "" )).attr("y", -i * 15)
    })
  })
